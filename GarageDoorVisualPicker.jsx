...
const DoorImage = ({ style, windows, color, size = “double” }) => {
...
const sizeCode = size === “single” ? “4C” : “8C”;
const resolvedImg = isWebp ? imgFile : imgFile.replace(“8C”, sizeCode);
...
<img
src={`${CDN_PNG}GLAZ-D-${sizeCode}_21.PNG`}
alt=“Windows”
...
<DoorImage style={doorStyle} windows={doorWindows} color={doorColor} size={size} />
</div>
</div>
);
};
          ...
          <>
            ...
            >
              {swipeIdx<STYLES.length-1&&(
                <div ref={bgCardRef} style={{position:"absolute",inset:0,background:C.card,borderRadius:20,border:`1px solid ${C.border}`,transform:"scale(0.94) translateY(14px)",opacity:0.35,overflow:"hidden",willChange:"transform, opacity",pointerEvents:"none"}}>
                  <div style={{...ss.doorBox,height:220}}>
                    <DoorImage style={STYLES[Math.min(swipeIdx+1,STYLES.length-1)].id} windows={false} size={sel.size} />
                  </div>
                </div>
              )}
              <div ref={node=>{activeCardRef.current=node;cardRef.current=node;}} style={{position:"absolute",inset:0,background:C.card,borderRadius:20,boxShadow:C.shadow,border:`1.5px solid ${C.border}`,transform:"translate3d(0,0,0) rotate(0deg) scale(1)",overflow:"hidden",willChange:"transform, box-shadow, border-color, opacity"}}>
                ...
                <div style={{...ss.doorBox,height:220}}>
                  <DoorImage style={cur.id} windows={false} size={sel.size} />
                </div>
        ...
        {options.map(s2=>(
          <button key={s2.id} style={{...ss.card,flexDirection:"column",alignItems:"stretch",padding:0,overflow:"hidden"}} onClick={()=>pick("style",s2.id)}>
            <div style={{...ss.doorBox,height:140}}>
              <DoorImage style={s2.id} windows={false} size={sel.size} />
            </div>
        ...
        {isWebpStyle ? (
          <>
            ...
            <div style={{...ss.doorBox,height:200,borderRadius:16,marginBottom:16}}>
              <DoorImage style={sel.style} windows={false} size={sel.size} />
            </div>
              ...
              {[true,false].map(w=>(
                <button key={String(w)} onClick={()=>setSel(p=>({...p,windows:w}))} style={{padding:0,borderRadius:16,overflow:"hidden",cursor:"pointer",border:sel.windows===w?`3px solid ${C.accent}`:`2px solid ${C.border}`,background:C.card,boxShadow:C.shadow,transition:"all 0.2s",WebkitTapHighlightColor:"transparent"}}>
                  <div style={{...ss.doorBox,height:110}}>
                    <DoorImage style={sel.style} windows={w} color={sel.color} size={sel.size} />
                  </div>
...
return (
<div style={ss.wrap}>
<style>{`@keyframes fadeUp { from { opacity: 0; transform: translateY(18px); } to { opacity: 1; transform: translateY(0); } } @keyframes pop { 0% { transform: scale(0.4); opacity: 0; } 60% { transform: scale(1.14); } 100% { transform: scale(1); opacity: 1; } } @keyframes doorReveal { 0% { opacity: 0; transform: scale(0.94); } 40% { opacity: 1; } 100% { transform: scale(1); } } @keyframes shimmer { 0% { background-position: 200% 0; } 100% { background-position: -200% 0; } } * { box-sizing: border-box; } input:focus { outline: none; } button:active { transform: scale(0.97); }`}</style>
