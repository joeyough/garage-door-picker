...
const DoorSVG = ({ style, windows, color = "#D1CCC5", w = 200, h = 160 }) => {
  const fill = color, dark = shadeColor(color, -25), light = shadeColor(color, 15);
  return (
    <svg viewBox={`0 0 ${w} ${h}`} width="100%" height="100%">
      <rect width={w} height={h} fill={fill} rx="3" />
      {style === "raised" && (<>
        {[0,1,2,3].map(r=>[0,1].map(c=>(<g key={`${r}${c}`}><rect x={6+c*(w/2-3)} y={5+r*(h/4-1)} width={w/2-12} height={h/4-8} rx="2" fill={dark}/><rect x={10+c*(w/2-3)} y={8+r*(h/4-1)} width={w/2-20} height={h/4-14} rx="1.5" fill={light}/></g>)))}
        {windows&&[0,1].map(c=>(<rect key={c} x={14+c*(w/2-3)} y={10} width={w/2-28} height={h/4-18} rx="1.5" fill="rgba(180,210,230,0.5)" stroke="rgba(100,140,170,0.4)" strokeWidth="1"/>))}
      </>)}
      {style === "flush" && (<>
        {[1,2,3,4,5].map(i=>(<line key={i} x1="0" y1={(i*h)/6} x2={w} y2={(i*h)/6} stroke={dark} strokeWidth="0.8"/>))}
        {windows&&[0,1,2,3].map(i=>(<rect key={i} x={12+i*(w/4-2)} y={8} width={w/4-16} height={h/6-12} rx="1" fill="rgba(180,210,230,0.5)" stroke="rgba(100,140,170,0.4)" strokeWidth="0.8"/>))}
      </>)}
      {style === "carriage" && (<>
        <line x1={w/2} y1="0" x2={w/2} y2={h} stroke={dark} strokeWidth="2.5"/>
        {[0,1].map(half=>(<g key={half}><line x1={w/4+(half*w)/2} y1="0" x2={w/4+(half*w)/2} y2={h} stroke={dark} strokeWidth="1"/>{[1,2].map(r=>(<line key={r} x1={half*(w/2)+2} y1={(r*h)/3} x2={(half+1)*(w/2)-2} y2={(r*h)/3} stroke={dark} strokeWidth="1"/>))}</g>))}
        {windows&&[0,1].map(half=>(<rect key={half} x={6+half*(w/2)} y={6} width={w/2-12} height={h/3-10} rx={h/6} fill="rgba(180,210,230,0.45)" stroke="rgba(100,140,170,0.4)" strokeWidth="1"/>))}
        <circle cx={w/2-8} cy={h/2} r="3" fill={dark}/><circle cx={w/2+8} cy={h/2} r="3" fill={dark}/>
      </>)}
      {style === "modern" && (<>
        {[0,1,2,3,4,5,6,7].map(i=>(<rect key={i} x="2" y={2+i*(h/8)} width={w-4} height={h/8-3} rx="1" fill={i%2===0?dark:light}/>))}
        {windows&&(<><rect x={w*0.55} y={4} width={w*0.4} height={h/8-5} rx="1" fill="rgba(180,210,230,0.5)" stroke="rgba(100,140,170,0.4)" strokeWidth="0.8"/><rect x={w*0.55} y={2+h/8+2} width={w*0.4} height={h/8-5} rx="1" fill="rgba(180,210,230,0.5)" stroke="rgba(100,140,170,0.4)" strokeWidth="0.8"/></>)}
      </>)}
    </svg>
  );
};
...
const STYLES = [
  { id:"raised", name:"Classic Raised Panel", tag:"Most Popular", tagC:C.accent, sub:"Traditional and timeless" },
  { id:"carriage", name:"Carriage House", tag:"Classic Charm", tagC:C.warm, sub:"Elegant barn-door feel" },
  { id:"flush", name:"Clean Flush", tag:"Sleek", tagC:"#57606A", sub:"Smooth, contemporary look" },
  { id:"modern", name:"Modern Plank", tag:"Trending", tagC:"#8B5E3C", sub:"Bold horizontal lines" },
];

const DOOR_COLORS = [
  { id:"white", name:"White", hex:"#F0EDE8" },{ id:"almond", name:"Almond", hex:"#E4D9C5" },
  { id:"sandstone", name:"Sandstone", hex:"#C9B99A" },{ id:"desert", name:"Desert Tan", hex:"#BDA67E" },
  { id:"brown", name:"Brown", hex:"#6B4226" },{ id:"black", name:"Black", hex:"#2A2A2A" },
  { id:"gray", name:"Slate Gray", hex:"#6B7B8D" },{ id:"green", name:"Hunter Green", hex:"#2D4A3E" },
...
const HousePreview = ({ doorStyle, doorColor, doorWindows, photoURL, size }) => {
  ...
  return (
    <div style={{ position:"relative", borderRadius:16, overflow:"hidden", background:"#D5CFC7", aspectRatio:"4/3", boxShadow:C.lift }}>
      ...
      <div style={{ position:"absolute", bottom:photoURL?"8%":0, left:photoURL?"50%":`${dx}px`, transform:photoURL?"translateX(-50%)":"none", width:photoURL?(size==="double"?"52%":"32%"):dw, height:photoURL?"38%":85, transition:"all 0.5s cubic-bezier(0.25,0.1,0.25,1)", filter:photoURL?"drop-shadow(0 2px 8px rgba(0,0,0,0.3))":"none", opacity:photoURL?0.92:1 }}>
        <DoorSVG style={doorStyle} windows={doorWindows} color={doorColor} w={300} h={225}/>
      </div>
    </div>
  );
};
                ...
                >
                  {swipeIdx<STYLES.length-1&&(
                    <div ref={bgCardRef} style={{position:"absolute",inset:0,background:C.card,borderRadius:20,border:`1px solid ${C.border}`,transform:"scale(0.94) translateY(14px)",opacity:0.35,overflow:"hidden",willChange:"transform, opacity",pointerEvents:"none"}}>
                      <div style={{height:210,background:"#EDE9E3",display:"flex",alignItems:"center",justifyContent:"center",padding:16,opacity:0.5}}>
                        <DoorSVG style={STYLES[Math.min(swipeIdx+1,STYLES.length-1)].id} windows={false} color="#C8C3BB" w={280} h={190}/>
                      </div>
                    </div>
                  )}
                  <div ref={node=>{activeCardRef.current=node;cardRef.current=node;}} style={{position:"absolute",inset:0,background:C.card,borderRadius:20,boxShadow:C.shadow,border:`1.5px solid ${C.border}`,transform:"translate3d(0,0,0) rotate(0deg) scale(1)",overflow:"hidden",willChange:"transform, box-shadow, border-color, opacity"}}>
                    ...
                    <div style={{height:210,background:"#EDE9E3",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
                      <DoorSVG style={cur.id} windows={false} color="#C8C3BB" w={280} h={190}/>
                    </div>
          ...
          <div style={SCREEN_STYLE}>
            ...
            {options.map(s2=>(
              <button key={s2.id} style={{...ss.card,flexDirection:"column",alignItems:"stretch",padding:0,overflow:"hidden"}} onClick={()=>pick("style",s2.id)}>
                <div style={{height:130,background:"#EDE9E3",display:"flex",alignItems:"center",justifyContent:"center",padding:14}}>
                  <DoorSVG style={s2.id} windows={false} color="#C8C3BB" w={260} h={120}/>
                </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
              {[true,false].map(w=>(
                <button key={String(w)} onClick={()=>setSel(p=>({...p,windows:w}))} style={{padding:0,borderRadius:16,overflow:"hidden",cursor:"pointer",border:sel.windows===w?`3px solid ${C.accent}`:`2px solid ${C.border}`,background:C.card,boxShadow:C.shadow,transition:"all 0.2s",WebkitTapHighlightColor:"transparent"}}>
                  <div style={{height:110,background:"#EDE9E3",display:"flex",alignItems:"center",justifyContent:"center",padding:10}}>
                    <DoorSVG style={sel.style} windows={w} color={sel.color} w={140} h={100}/>
                  </div>
      ...
      case "insulation":
        return (
          <div style={SCREEN_STYLE}>
            ...
            {sel.insulation && (
              <div style={{ textAlign:"center", fontSize:14, fontWeight:600, color:C.accent, marginBottom:4, marginTop:-4, animation:"fadeUp 0.2s ease-out" }}>
                Selected: {insSelectedLabel}
              </div>
            )}

            {sel.insulation && (
              <button style={{ ...ss.btn, ...ss.pri, marginTop:8, marginBottom:14, animation:"fadeUp 0.25s ease-out" }} onClick={next}>
                {insCtaLabel}
              </button>
            )}
              ...
              <div style={{background:C.card,borderRadius:16,padding:22,border:`1px solid ${C.border}`,boxShadow:C.shadow,animation:"fadeUp 0.3s ease-out",marginTop:12}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20}}>
                  <div style={{textAlign:"center"}}>
                    <div style={{background:"#EDEBE7",borderRadius:12,padding:"20px 12px",marginBottom:10}}>
                      <div style={{width:"70%",height:10,background:"#BBB",margin:"0 auto",borderRadius:5,boxShadow:"inset 0 1px 2px rgba(0,0,0,0.1)"}}/>
                    </div>
                    <div style={{fontSize:14,fontWeight:700}}>Standard</div>
                    <div style={{fontSize:12,color:C.dim}}>Single layer of steel</div>
                  </div>
                  <div style={{textAlign:"center"}}>
                    <div style={{background:C.accentSoft,borderRadius:12,padding:"20px 12px",marginBottom:10,border:`1px solid ${C.accent}33`}}>
                      <div style={{display:"flex",flexDirection:"column",gap:3,alignItems:"center"}}>
                        <div style={{width:"70%",height:5,background:"#7BAE7E",borderRadius:3}}/>
                        <div style={{width:"70%",height:8,background:"#A8D4AB",borderRadius:4,boxShadow:"inset 0 1px 2px rgba(0,0,0,0.05)"}}/>
                        <div style={{width:"70%",height:5,background:"#7BAE7E",borderRadius:3}}/>
                      </div>
                    </div>
                    <div style={{fontSize:14,fontWeight:700,color:C.accent}}>Insulated</div>
                    <div style={{fontSize:12,color:C.dim}}>3-layer sandwich</div>
                  </div>
                </div>
                <div style={{marginBottom:20}}>
                  {[{icon:"💪",text:"Stronger and more durable — resists dents"},{icon:"🤫",text:"Quieter — less noise opening and closing"},{icon:"🌡️",text:"Helps with temperature — keeps heat and cold out"}].map((b,i)=>(
                ...
                <div style={{background:C.bg,borderRadius:12,padding:16,marginBottom:14,marginTop:16}}>
                  ...
                  {[{level:"R-6",label:"Basic",desc:"Entry-level insulation",bar:25,color:C.border},{level:"R-12",label:"Best Value",desc:"Most popular choice",bar:60,color:C.accent,highlight:true},{level:"R-18",label:"Maximum",desc:"Extreme climates only",bar:90,color:C.dim}].map((tier,i)=>(
                    <div key={i} style={{padding:"10px 0",borderBottom:i<2?`1px solid ${C.border}`:"none"}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                        <div style={{display:"flex",alignItems:"center",gap:8}}>
                          <span style={{fontSize:14,fontWeight:700,color:tier.highlight?C.accent:C.text}}>{tier.level}</span>
                          <span style={{fontSize:13,fontWeight:500,color:tier.highlight?C.accent:C.dim}}>{tier.label}</span>
                          {tier.highlight&&<span style={ss.tag(C.accent)}>Popular</span>}
                        </div>
                      </div>
                      <div style={{height:6,borderRadius:3,background:"#E0DCD6",overflow:"hidden",marginBottom:3}}>
                        <div style={{height:"100%",width:`${tier.bar}%`,background:tier.color,borderRadius:3,transition:"width 0.5s ease"}}/>
                      </div>
                      <div style={{fontSize:12,color:C.dim}}>{tier.desc}</div>
                    </div>
                  ))}
                </div>
      ...
      case "color":
        return (
          <div style={{ ...SCREEN_STYLE, padding: "20px 22px 132px" }}>
            ...
            <div style={{ textAlign: "center", marginBottom: 8, fontSize: 15, fontWeight: 600, color: C.accent, transition: "all 0.3s" }}>
              Selected: {sel.colorName}
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
              {DOOR_COLORS.map((c) => {
                const active = sel.color === c.hex;
                return (
                  <button
                    key={c.id}
                    onClick={() => {
                      setSel((p) => ({ ...p, color: c.hex, colorName: c.name }));
                      setColorPulse((p) => p + 1);
                    }}
                    style={{
                      padding: 10, borderRadius: 14, cursor: "pointer",
                      border: active ? `3px solid ${C.accent}` : `2px solid ${C.border}`,
                      background: active ? C.accentSoft : C.card,
                      boxShadow: active ? `0 0 0 3px ${C.accentSoft}` : C.shadow,
                  ...
                  >
                    <div style={{
                      width: 40, height: 40, borderRadius: 12, background: c.hex,
                      border: c.hex === "#F0EDE8" ? "1px solid #ddd" : "1px solid transparent",
                      boxShadow: "inset 0 1px 3px rgba(0,0,0,0.12)", transition: "transform 0.2s",
                    }} />
                    <span style={{ fontSize: 10, fontWeight: active ? 700 : 600, color: active ? C.accent : C.dim, textAlign: "center", lineHeight: 1.2 }}>
                      {c.name}
                    </span>
                  </button>
                );
            ...
            <div style={{
              position: "sticky", bottom: 12, marginTop: 14, paddingTop: 10,
              background: "linear-gradient(to bottom, rgba(246,241,235,0), rgba(246,241,235,0.92) 28%, rgba(246,241,235,1) 100%)",
            }}>
              <button style={{ ...ss.btn, ...ss.pri, marginTop: 0, marginBottom: 0 }} onClick={next}>
